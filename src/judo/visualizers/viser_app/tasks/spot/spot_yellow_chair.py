...
class SpotYellowChair(SpotBase[SpotYellowChairConfig]):
    ...
    def __init__(self) -> None:
        self.model_filename = str(MODEL_PATH / "xml/spot_yellow_chair.xml")
    ...
    def reward(
        ...
        object_linear_velocity = states[..., -6:-3]

        object_y_axis = sensors[..., 3:6]
        body_to_object = sensors[..., 6:9]
        ...
        controls_reward = -config.w_controls * np.linalg.norm(
            controls[..., :3], axis=-1
        ).mean(-1)
    ...
    def actuator_ctrlrange(self) -> np.ndarray:
        ...
        lower_bound = np.concatenate(
            (-1.0 * np.ones(3), ARM_UNSTOWED_POS - np.array([1, 1, 0.7, 1, 1, 1, 0]))
        )
