...
from judo.mujoco_helpers.utils import count_trace_sensors, get_mesh_data
...
class ViserMjModel:
    ...
    def set_traces(
        ...
        if traces is None:
            ...
            # check if the number of received traces matches the current number of splines.
...
def add_segments(
    ...
    return target.scene.add_line_segments(
        name, points, rgb, line_width, quat, pos, visible
    )
